...
const formTitle = computed(() => {
  return editedIndex.value === -1 ? 'Add New One-Time Expense' : 'Edit One-Time Expense'
})
...
const fetchExpenses = () => {
  ...
  return new Promise(resolve => {
    setTimeout(() => {
      resolve([
        { id: 'OT1', name: 'Office Equipment', amount: 1500 },
        { id: 'OT2', name: 'Furniture', amount: 3000 }
      ])
    }, 1000)
  }).then(data => {
...
const createExpense = expense => {
  ...
  return new Promise(resolve => {
    setTimeout(() => {
      ...
      resolve({ ...expense, id: `OT${expenses.value.length + 1}` })
    }, 500)
  })
}
