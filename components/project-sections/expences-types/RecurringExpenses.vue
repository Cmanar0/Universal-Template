<template>
  <div class="mb-6 md:pl-8">
    <h2 class="text-xl pb-4">Recurring Expenses</h2>
    <v-btn color="success" size="large" @click="openDialog" class="mb-6">Add New Recurring Expense</v-btn>
      ...
      <v-card>
        ...
        <v-card-text>
          <v-form ref="refForm" v-model="isFormValid">
            <v-text-field v-model="editedItem.name" :rules="[rules.required]" label="Expense Name" color="#1697F6" variant="outlined" dense class="pt-2"></v-text-field>
            <v-text-field
...
import { VBtn, VDataTable, VDialog, VCard, VCardTitle, VCardText, VCardActions, VTextField, VForm, VSelect, VSpacer, VIcon, VSkeletonLoader } from 'vuetify/components'
...
// Options for expense frequencies
...
const formTitle = computed(() => {
  return editedIndex.value === -1 ? 'Add New Recurring Expense' : 'Edit Recurring Expense'
})
...
const fetchExpenses = () => {
  ...
  return new Promise(resolve => {
    setTimeout(() => {
      resolve([
        { id: 'RE1', name: 'Software Subscription', amount: 50, frequency: 'Monthly' },
        { id: 'RE2', name: 'Office Rent', amount: 2000, frequency: 'Monthly' }
      ])
    }, 1000)
  }).then(data => {
...
const createExpense = expense => {
  ...
  return new Promise(resolve => {
    setTimeout(() => {
      ...
      resolve({ ...expense, id: `RE${expenses.value.length + 1}` })
    }, 500)
  })
}
