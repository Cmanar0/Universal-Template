  ...
  <div v-if="notifications.length" class="fixed top-4 right-4 z-50">
    ...
    >
      <div
        ...
        :key="notification.id"
        class="notification bg-blue-100 p-4 rounded-lg shadow-md flex items-center justify-between"
      >
        <div>
          <h3 class="text-lg font-bold text-blue-700 mb-2">
            {{ notification.title }}
          </h3>
          <p class="text-blue-600">{{ notification.message }}</p>
        </div>
        <button
...
import state, { removeNotification } from '@/stores/notificationStore' // Import the state and removeNotification
